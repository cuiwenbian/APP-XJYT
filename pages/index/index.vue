	...
	export default {
		...
		mounted() {
			...
			uni.request({
				...
				success: res => {
					that.andri = uni.getStorageSync('version'); //当前应用版A本
					console.log(res)
					that.version = res.data[1].version; //获取服务器版本
					console.log('本地版本号'+that.version)
					console.log('服务器版本号'+that.andri)
					if (that.andri > that.version) { //本地与服务器比较
						that.According = true;
						that.remark = res.data[0].remark;
						that.daern = res.data[0].must; //当前是否必须？
						if (that.daern == 1) { //当前必须的话
							that.diro = false; //diro？好像没有呗使用
						}
						uni.hideTabBar()
					}
				}
